...
function copySubscription(filename) {
  // Корректная склейка пути независимо от наличия index.html или слэша на конце
  ...
  const subUrl = window.location.origin + basePath + 'sub/' + filename;

  navigator.clipboard.writeText(subUrl).then(() => {
    showToast('Ссылка скопирована в буфер!');

    const qrContainer = document.getElementById('qr-container');
    const currentUrlSpan = document.getElementById('current-sub-url');
    const qrImg = document.getElementById('qrcode');

    qrContainer.style.display = 'block';
    currentUrlSpan.textContent = subUrl;

    QRCode.toDataURL(subUrl, { width: 200, margin: 1 }, (err, url) => {
      if (err) {
